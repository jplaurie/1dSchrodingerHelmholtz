...
static double k,k2, psi_hat2,flux;
...
/* compute the nonlinear term based on model */		
	...
	if( FLAG_MODEL_TYPE == "SHE"){
    	...
    	fftw_execute(FFT);
    	psi_hat_M /= double(M);
    	psi_hat_M %= pow( 1.0 + (beta*k2_vector) ,-0.5);
...
void computeFlux(cx_rowvec psi_hat, cx_rowvec L,cx_rowvec & psi_M ,cx_rowvec & psi_hat_M, rowvec & wave_flux ,rowvec & energy_flux,fftw_plan FFT, fftw_plan IFFT ){
    ...
    psi2_hat = nonlinear(psi_hat,0.0, L , psi_M, psi_hat_M, FFT, IFFT);
    k=0.0;
    wave_flux.zeros();
    energy_flux.zeros();
    ...
    for(int i = 0; i< N/2; i++){
            ...
            flux = -2.0 * ( real(psi_hat(i))*real(psi2_hat(i)) + imag(psi_hat(i))*imag(psi2_hat(i)) );
  
			//integrate from 0 to l
            for(int l=ib; l < N/2 +1; l++){
            	wave_flux(l) += flux;
            	energy_flux(l) -= c * k * k * flux;
			}
            ...
            flux = -2.0 * ( real(psi_hat(N-i-1))*real(psi2_hat(N-i-1)) + imag(psi_hat(N-i-1))*imag(psi2_hat(N-i-1)) );
